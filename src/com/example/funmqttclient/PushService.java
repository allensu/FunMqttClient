...
public class PushService extends Service {
	...
	//private static final String MQTT_HOST = "202.133.233.212"; // dev2
	private static final String MQTT_HOST = "202.133.233.210"; // tpe01
	...
	// MQTT broker.
	public static String MQTT_CLIENT_ID = "fun";
	...
	// Notification title
	public static String NOTIF_TITLE = "FunCube Message";
	// Notification id
...
//		}.start();
	...
	private class MQTTConnection implements MqttSimpleCallback {
		...
		public ArrayList<String> recvIds = new ArrayList<String>();
		MqttPublishDel mqttPublishDel = new MqttPublishDel();
	...
	private class MqttPublishDel extends Thread {
		...
		public void run() {
			try {
				while (true) {
					...
					int nowCount = mConnection.recvIds.size();
					for (int i = 0; i < nowCount; i++) {
						try {
							String result2Server = "0:del:"
									+ mConnection.recvIds.get(0);
							mConnection.publishToTopic("$sys/", result2Server);
							mConnection.recvIds.remove(0);
						} catch (Exception ex) {
							log("MqttException"
